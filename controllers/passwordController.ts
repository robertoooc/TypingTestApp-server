...
const checkToken = async (token: string) => {
  try {
    ...
    const expirationTime =
      resetToken.expiration.getTime() + tokenExpirationTime;
...
const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    const existingToken = await ResetToken.find({ email: encryptedEmail });
    if (existingToken) {
      existingToken.forEach(async (token) => {
    ...
    const expiration = moment()
      ...
      .add(tokenExpirationTime, 'milliseconds')
      .toDate();

    await ResetToken.create({ token, expiration, email: encryptedEmail });
